...
const Formulario = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    console.log(data);
    await api.post('/tarifa', data).then((result) => {
  ...
  return (
    <Wrapper>
          ...
          <li>
            ...
            <select id='plano' name='plano' onChange={handleInputChange}>
              ...
              {planos.map((plano, index) => (
                <option key={index} value={JSON.stringify(plano)}>
                  {plano.nome}
                </option>
              ))}
            </select>
      ...
      {pageResult && (
        <Result>
          <img src={close} alt='close' onClick={() => closeResult()} />
          <div>
            <p>
              O valor da sua ligação <strong>com</strong> o plano{' '}
              <strong>FaleMais</strong>: <strong>{resultado[0][1]}</strong>
            </p>
            <p>
              O valor da sua ligação sem o plano FaleMais:{' '}
              <strong>{resultado[1][1]}</strong>
            </p>
          </div>
        </Result>
      )}
